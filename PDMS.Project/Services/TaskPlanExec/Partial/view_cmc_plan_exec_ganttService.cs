            ...
            if (getinfo.Count != 0)
            {
                ...
                try
                {
                    ...
                    foreach (var item in taskInfo)
                    {
                        ...
                        data.start_date = item.Key.start_date == null ? "" : item.Key.start_date.ToString("yyyy-MM-dd HH:mm:ss");
                        data.end_date = item.Key.end_date == null ? "" : item.Key.end_date.ToString("yyyy-MM-dd HH:mm:ss");
                        data.parent = index;
