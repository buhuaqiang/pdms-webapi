...
let extension = {
            ...
            render () {
                return [
                    h(resolveComponent('el-alert'), {
                        ...
                        closable: false, title: '界面下拉框、多選、checkbox等數據源都在此處維護，也是代碼生成器中的數據源'
                    }, ''),
                ]
            }
        },
    ...
    methods: { //事件擴展
        onInit () {
            //點擊單元格編輯與結束編輯(默認是點擊單元格編輯，鼠標離開結束編輯)
            ...
            this.editFormOptions.forEach(x => {
                x.forEach(item => {
                    ...
                    if (item.field == "DbSql") {
                        item.placeholder = "如果從數據庫加載數據源，請按此格式配置sql語句：select orderType as key,orderName as value from order  如果需要根據用戶信息加載數據源，請配置好此sql,再修改后臺DictionaryHandler.GetCustomDBSql方法";
                    }
                })
            })
