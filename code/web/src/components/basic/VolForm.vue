<template>
  <el-form
    style="display: inline-block; width: 100%; padding-left: 9px"
    :inline="true"
    ref="volform"
    @submit.prevent
    :model="formFields"
    :label-width="labelWidth"
    :label-position="$global.lang ? 'top' : 'left'"
    :rules="rules"
    :class="[$global.lang ? 'form-lang' : 'form-normal']"
  >
    <template v-for="(row, findex) in formRules" :key="findex">
      <div class="vol-form-item" style="width: 100%">
        <el-form-item
          :label="$ts(item.title) + ($global.lang ? '' : '：')"
          v-show="!item.hidden"
...
<script>
const rule = {
  change: ["checkbox", "select", "date", "datetime", "drop", "radio", "cascader"], // 2020.05.31增加級聯類型
    ...
    const initFormRules = (init) => {
      ...
      props.formRules.forEach((row, xIndex) => {
        ...
        row.forEach((item, yIndex) => {
          ...
          // 目前只支持select單選遠程搜索，remote遠程從后臺字典數據源進行搜索，url從指定的url搜索
      ...
      volform.value.validate((valid) => {
        if (!valid) {
          appContext.config.globalProperties.$message.error(
            appContext.config.globalProperties.$ts("數據驗證未通過!")
          );
          result = false;
  ...
  methods: {
    getColWidth(item) {
      ...
      this.formRules.forEach((row, xIndex) => {
        //2022.05.06 追加表單中隱藏的元素不參與動態計算表單寬度
    ...
    // 是否為圖片文件等格式并對字段的轉換成數組：[{name:'1.jpg',path:'127.0.0.1/ff/1.jpg'}]
    ...
    validatorPhone(ruleOption, value, callback) {
      ...
      if ((value + "").trim().length < 6) {
        return callback(new Error(this.$ts("密碼長度不能小于6位")));
      }
      callback();
    },
    ...
    getRule(item, formFields) {
        ...
        return {
          validator: (rule, val, callback) => {
            ...
            if (item.required && !this.isReadonly(item) && (!val || !val.length)) {
              return callback(
                new Error(this.$ts(item.type == "img" ? "請上傳照片" : "請上傳文件"))
              );
            }
            return callback();
          },
          ...
          validator: (ruleObj, value, callback) => {
            ...
            if (ruleObj.type == "number") {
              if (!rule.number.test(value)) {
                ruleObj.message = this.$ts([ruleObj.title, "只能是整數"], true);
                return callback(new Error(ruleObj.message));
              }
            } else {
              if (!rule.decimal.test(value)) {
                ruleObj.message = this.$ts([ruleObj.title, "只能是數字"], true);
                return callback(new Error(ruleObj.message));
              }
            }
            if (
      ...
      if (item.type == "radio") {
        return {
          required: item.required,
          message: this.$ts(["請選擇", item.title], true),
          trigger: "change",
          type: "string",
        };
      }
        ...
        return {
          ...
          validator: (rule, val, callback) => {
            ...
            if (!val || (item.range && !val.length)) {
              return callback(new Error(this.$ts("請選擇")));
            }
            return callback();
          },
        };
