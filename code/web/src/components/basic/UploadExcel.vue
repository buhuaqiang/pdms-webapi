<template>
  <div class="upload-container">
    ...
    <div class="alert">
      <el-alert :title="$ts('只能上傳excel文件,文件大小不超過5M')" type="warning" :closable="false" show-icon
        ></el-alert
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    upload() {
      let _url = this.url;
      if (!_url) {
        return this.$Message.error(this.$ts("沒有配置好Url"));
      }
      ...
      this.http.post(_url, formData).then(
        (x) => {
          ...
          this.message =this.$ts(x.Msg||"導入成功");
