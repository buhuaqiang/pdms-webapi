...
export default {
  ...
  props: {
    ...
    fileInfo: {
      //用于接收上傳的文件，也可以加以默認值，顯示已上傳的文件，用戶上傳后會覆蓋默認值
      type: Array,
      default: () => {
        return [];
  ...
  data() {
    return {
      ...
      changed: false, //手動上傳成功后禁止重復上傳，必須重新選擇
      model: true,
      files: [],
      bigImg: '',
  ...
  methods: {
    ...
    async upload(vail) {
      ...
      //增加上傳時自定義參數，后臺使用獲取Utilities.HttpContext.Current.Request.Query["字段"]
      let params={};
