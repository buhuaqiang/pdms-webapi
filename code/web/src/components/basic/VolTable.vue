      ...
      >
        ...
        <template #default="scope">
          ...
          <template v-else>
            ...
            ></div>
            <!-- 2021.11.18修復table數據源設置為normal后點擊行$event缺失的問題 -->
            <div
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      // 目前只支持從后臺返回的summaryData數據
      summaryData: [],
      summaryIndex: {},
      remoteColumns: [], // 需要每次刷新或分頁后從后臺加載字典數據源的列配置
      cellStyleColumns: {}, // 有背景顏色的配置
      fxRight: false, //是否有右邊固定表頭
      selectRows: [], //當前選中的行
      isChrome: false,
      //vol-table帶數據源的單元格是否啟用tag標簽(下拉框等單元格以tag標簽顯示)
  ...
  methods: {
    ...
    rowClick(row, column, event) {
      ...
      if (this.clickEdit && this.edit.rowIndex != -1) {
        ...
        //當正在編輯，且點擊到其他行時，在原編輯的行結束編輯后，觸發新行的rowClick事件
        //正在編輯時，禁止出發rowClick事件
        if (this.edit.rowIndex == -1) {
          this.$emit("rowClick", { row, column, event });
        }
    ...
    rowEndEdit(row, column, event) {
      ...
      if (this.edit.rowIndex != -1) {
        //2022.06.26修復表格內容切換后行數不一致時不能編輯的問題
    ...
    delRow() {
      ...
      if (rows.length == 0) return this.$Message.error(this.$ts("請選擇要刪除的行!"));

      let data = this.url ? this.rowData : this.tableData;
      let indexArr = this.getSelectedIndex();
      if (indexArr.length == 0) {
        return this.$Message.error("刪除操作必須設置VolTable的屬性index='true'");
      }
    ...
    formatter(row, column, template) {
      ...
      if (!column.bind || !column.bind.data) {
       ...
       if (column.edit &&( column.edit.type == "selectList"|| column.edit.type == "checkbox" || column.edit.type == "treeSelect")) {
