...
let methods = {
  ...
  getCurrentAction() {
    ...
    return (
      '(' +
      this.$ts(this.currentAction == this.const.ADD ? '新增' : '編輯') +
      ')'
    );
  },
  ...
  extendBtn(btns, source) {
    ...
    //source通過在表的擴展js文件中buttons對應按鈕的屬性index決定按鈕所放位置
    source.forEach((x) => {
  ...
  initBoxButtons() {
    ...
    //只有明細表，將明細表也設置為不可能編輯，并且不顯示添加行、刪除行
    if (!saveBtn) {
    ...
    this.detailOptions.buttons.push(
      ...[
        ...
        //注意需要重寫后臺明細表接口的導入與下載模板、導出的權限,Sys_DictionaryListController.cs/SellOrderListController.cs
        {
          type: 'danger',
          plain: true,
          name: '導入',
  ...
  del(rows) {
    ...
    if (!rows || rows.length == 0)
      return this.$error(this.$ts('請選擇要刪除的行!'));
    let delKeys = rows.map((x) => {
      return x[this.table.key];
    });
    if (!delKeys || delKeys.length == 0)
      return this.$error(this.$ts('沒有獲取要刪除的行數據!'));
    //刪除前
    if (!this.delBefore(delKeys, rows)) {
      return;
    }
    let tigger = false;
    this.$confirm(this.$ts('確認要刪除選擇的數據嗎?'), this.$ts('警告'), {
      ...
      this.http
        .post(url, delKeys, this.$ts('正在刪除數據') + '....')
        .then((x) => {
          if (!x.status) return this.$error(x.message);
          this.$success(x.message);
          //刪除后
  ...
  async initBox() {
    ...
    if (this.newTabEdit) {
      this.$tabs.open({
        text: this.$ts(this.table.cnName) + '(' + (value ? this.$ts('編輯') : this.$ts('新建')) + ')',
  ...
  async linkData(row, column) {
    this.boxOptions.title =
      this.$ts(this.table.cnName) + '(' + this.$ts('編輯') + ')';
  ...
  async add() {
    this.boxOptions.title =
      this.$ts(this.table.cnName) + '(' + this.$ts('新建') + ')';
    //新建
    this.currentAction = this.const.ADD;
    this.currentRow = {};
    if (!(await this.initBox())) return;
  ...
  export(isDetail) {
    ...
    //2021.01.08優化導出功能
    $http
      .post(url, param, 'loading....', { responseType: 'blob' })
      .then((content) => {
  ...
  audit() {
    ...
    if (!auditStatus) {
      return this.$message.error(
        this.$ts(`表必須包括審核字段【AuditStatus】,并且是int類型`)
      );
    }
  ...
  saveAudit(params, rows, callback) {
    ...
    let url = `${this.getUrl(this.const.AUDIT)}?auditReason=${
      params.reason
    ...
    formOptions.forEach((item) => {
      item.forEach((d) => {
        ...
        if (keys.indexOf(d.dataKey) == -1) {
          ...
          keys.push(d.dataKey);
          //2020.05.03修復查詢表單與編輯表單type類型變成強一致性的問題
          //this.dicKeys.push({ dicNo: d.dataKey, data: [], type: d.type });
          //  2020.11.01增加iview組件Cascader數據源存儲
          let _dic = {
            dicNo: d.dataKey,
            data: [],
            fileds: [d.field],
        ...
        if (d.type != 'cascader') {
          //2020.01.30移除內部表單formOptions數據源配置格式data.data，所有參數改為與組件api格式相同
          Object.assign(
            d,
  ...
  //初始table與明細表的數據源指向dicKeys對象，再去后臺加載數據源
    ...
    dic.forEach((d) => {
      ...
      this.dicKeys.forEach((x) => {
        ...
        if (x.type == 'cascader' || x.type == 'treeSelect') {
          ...
          //2022.04.04增加級聯字典數據源刷新后table沒有變化的問題
          ...
          //2020.06.06，如果是selectList數據源使用的自定義sql并且key是數字，強制轉換成字符串
          if (
            x.e_type == 'selectList' &&
            d.data.length > 0 &&
  ...
  initDicKeys() {
    ...
    //初始化datatable表數據源,默認為一個空數組,dicKeys為界面所有的數據字典編號
  ...
  setFiexdColumn(columns, containerWidth) {
    //計算整個table的寬度，根據寬度決定是否啟用第一行顯示的列為固定列
    //2021.09.21移除強制固定第一列
    // let columnsWidth = 0;
    // columns.forEach(x => {
    //   if (!x.hidden && x.width) {
  ...
  initBoxHeightWidth() {
    ...
    if (this.detail.columns && this.detail.columns.length > 0) {
      ...
      //計算彈出框整個table的寬度，根據寬度決定是否啟用第一行顯示的列為固定列
  ...
  tableBeginEdit(row, column, index) {
    //2021.03.19是否開啟查詢界面表格雙擊編輯結束方法,返回false不會結束編輯
  ...
  loadTreeChildren(tree, treeNode, resolve) {
    //樹形結構加載子節點(2021.05.02),在onInit中設置了rowKey主鍵字段后才會生效
    return resolve([]);
  },
  importDetailAfter(data) {
    //2022.01.08增加明細表導入后處理
  },
  importExcelAfter(data) {
    //2022.01.08增加明細表導入后方法判斷
    ...
    if (this.boxModel) {
      ...
      this.importDetailAfter(data); //增加明細表導入后處理
