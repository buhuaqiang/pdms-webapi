          ...
          <template #footer>
            ...
            <div v-show="!isBoxAudit">
              <el-button
                ...
                @click="onClick(btn.onClick)"
              >
                <i :class="btn.icon"> {{ $ts(btn.name) }}</i>
              </el-button>
              <el-button
                size="small"
                type="primary"
...
var vueParam = {
  ...
  data() {
    return {
      ...
      continueAddName: "保存后繼續添加", //2021.04.11按鈕名稱
      // detailUrl: "",
      detailOptions: {
        //彈出框從表(明細)對象
        //從表配置
        ...
        height: 0, //默認從表高度
        textInline: true, //明細表行內容顯示在一行上，如果需要換行顯示，請設置為false
        doubleEdit: true, //使用雙擊編輯
        clickEdit: false, //是否開啟點擊單元格編輯，點擊其他行時結束編輯
        currentReadonly: false, //當前用戶沒有編輯或新建權限時，表單只讀(可用于判斷用戶是否有編輯或新建權限)
        //開啟編輯時
      ...
      textInline: true, //table內容超出后是否不換行2020.01.16
      ...
      }, //saveClose新建或編輯成功后是否關閉彈出框//彈出框的標簽寬度labelWidth
      editor: {
        uploadImgUrl: "", //上傳路徑
        upload: null, //上傳方法
      },
  ...
  created: function () {
    ...
    this.initBoxButtons(); //初始化彈出框與明細表格按鈕
    ...
    this.onInited(); //初始化后，如果需要做其他處理在擴展方法中覆蓋此方法
  },
