  ...
  >
    <div class="audit-model-content" :style="{ height: height - 70 + 'px' }">
      ...
      <div v-show="activeName == 'audit' || !hasFlow" class="audit-content">
        ...
        <div
          class="fx-right"
          :style="{ width: !hasFlow ? '100%' : '400px' }"
          v-if="isCurrentUser || !hasFlow"
        >
          <div v-if="!hasFlow">
            <el-alert
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const columns = reactive([
      ...
      { title: "備注", field: "remark", width: 120 },
    ]);
    ...
    const getAuditInfo = () => {
      ...
      http.post(url, ids, true).then((result) => {
        ...
        if (!hasFlow.value) {
          ...
          if (!checkStatus) {
            proxy.$message.error($ts("只能選擇待審批或審核中的數據"));
            return;
          }
