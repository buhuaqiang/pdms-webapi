...
let serviceFilter = {
  ...
  searchAfter (param, result) { //查詢ViewGird表數據后param查詢參數,result回返查詢的結果
  ...
  searchDetailAfter (param, data) {//查詢從表后param查詢參數,result回返查詢的結果
    // console.log(this.detailOptions.cnName + '觸發loadDetailTableAfter');
    return true;
  },
  delBefore (ids, rows) { //查詢界面的表刪除前 ids為刪除的id數組,,rows刪除的行
    return true;
  },
  delAfter (result) {//查詢界面的表刪除后
    return true;
  },
  delDetailRow (rows) { //彈出框刪除明細表的行數據(只是對table操作，并沒有操作后臺)
    return true;
  },
  ...
  resetAddFormBefore () { //重置新建表單前的內容
    return true;
  },
  resetAddFormAfter () { //重置新建表單后的內容
    return true;
  },
  resetUpdateFormBefore () { //重置編輯表單前的內容
    return true;
  },
  resetUpdateFormAfter () { //重置編輯表單后的內容
    return true;
  },
  modelOpenBefore (row) { //點擊編輯/新建按鈕彈出框前，可以在此處寫邏輯，如，從后臺獲取數據

  },
  modelOpenAfter (row) {  //點擊編輯/新建按鈕彈出框后，可以在此處寫邏輯，如，從后臺獲取數據
