<template>
  <div id="vol-container" :class="['vol-theme-' + theme]">
    ...
    <div class="vol-container" :style="{ left: menuWidth - 1 + 'px' }">
      <div class="vol-header">
        ...
        <div class="header-info">
          <div class="h-link" style="margin-right: 20px;" v-if="permissionInited">
            <service-select :color="color" v-if="$global.db"></service-select>
          </div>
          <div class="h-link" v-if="permissionInited && $global.lang">
          ...
          <div>
            <img class="user-header h-link" :src="userImg" :onerror="errorImg" />
          </div>
          <div class="user h-link">
            <div class="user-name">{{ userName }}</div>
            <div id="index-date"></div>
          </div>
          <div class="settings h-link">
          ...
          <ul :style="{ left: menuLeft + 'px', top: menuTop + 'px' }" class="contextMenu">
            <li v-show="visibleItem.all">
              <el-button link @click="closeTabs()">
                ...
                {{
                  $ts(navigation.length == 2 ? '關閉菜單' : '關閉所有')
                }}</el-button>
            </li>
            <li v-show="visibleItem.left">
              <el-button link @click="closeTabs('left')"><i class="el-icon-back"></i>{{ $ts('關閉左邊') }}</el-button>
            </li>
            ...
            <li v-show="visibleItem.other">
              <el-button link @click="closeTabs('other')"><i class="el-icon-right"></i>{{ $ts('關閉其他') }}
              </el-button>
            </li>
          </ul>
        </div>
    ...
    <el-drawer :title="$ts('選擇主題')" size="340px" v-model="drawer_model" direction="rtl" destroy-on-close>
      <div class="theme-selector">
        <div @click="changeTheme($ts(item.name))" class="item" v-for="(item, index) in theme_color" :key="index"
...
var imgUrl = require('@/assets/imgs/default_header.png');
var $this;
var $interval;
var $indexDate;
import {
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const theme_color = reactive([
      { name: 'dark', color: '#272929', dark: true },
    ...
    const links = ref([
      { text: "框架文檔", path: "http://v2.volcore.xyz/document/guide", id: -1, icon: "el-icon-document", left: true },
      { text: "App移動端", path: "http://v2.volcore.xyz/app/home", id: -1, icon: "el-icon-mobile", left: true },
    ...
    const color = ref('')
    const getColor = () => {
      color.value = theme_color.some(x => { return theme.value == x.name && x.dark }) ? '#373636' : '#ffff'
    }
    const errorImg = ref('this.src="' + require('@/assets/imgs/default_header.png') + '"');
    ...
    const changeTheme = (name) => {
      ...
      getColor();
      localStorage.setItem('vol:pro:theme', name);
    };
    const to = (item) => {
    ...
    const getItem = () => {
      ...
      if (item.meta && item.meta.name) {
        ...
        if (item.meta.edit) {
          name += router.currentRoute.value.query.id ? '(編輯)' : '(新建)';
        }
        item = {
          name: name,
     ...
     */
    const created = () => {
      let _theme = localStorage.getItem('vol:pro:theme');
      if (_theme) {
        theme.value = _theme;
      }
      getColor();
      ...
      http.get('api/menu/getTreeMenu', {}, true).then((result) => {
        ...
        data.forEach((d) => {
          if (d.url && d.url.indexOf('?') != -1) {
            ...
            _arr = _arr[1].split('&');
            let queryObj = {};
            for (let i = 0; i < _arr.length; i++) { // 遍歷參數
              if (_arr[i].indexOf('=') != -1) { // 如果參數中有值
