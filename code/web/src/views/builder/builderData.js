...
// 數據庫名配置
//key屬性必須與BlueWhale.Cloud.Core中EFDbContext的名字一致
//系統庫SysDbContext的名字不要改，固定是這個，其他可以修改
const dbOptions = [
  { key: 'SysDbContext', value: 'SysDbContext(系統庫)' },
  { key: 'ServiceDbContext', value: 'ServiceDbContext(業務庫)' },
  { key: 'TestDbContext', value: 'TestDbContext(測試庫)' },
  { key: '自定義DbContext', value: '自定義DbContext(測試庫2)' }
];
...
let dataType = [
  ...
  { key: 'treeSelect', value: '樹形下拉框treeSelect' }, 
...
let data = {
  form: {
    fields: {
      ...
      editType:null,//編輯模式
      userPermissionDesc:
        '開啟后當前用戶只能操作自己(與下級角色)創建的數據,如:查詢、刪除、修改等操作'
    },
    addOptions: [
      [
        {
          ...
          type: 'number',
          placeholder:
            '放在【代碼生成配置】列表的文件夾ID下,如果填入【0】就是一級目錄'
        }
     ,
        {
          title: '項目類庫',
          field: 'namespace',
          placeholder:
            '代碼生成后的所在類庫(可以自己提前在后臺項目中創建一個.netcore類庫)',
          type: 'select',
          required: true,
          data: []
        }
     ...
     ,
        {
          ...
          dataKey:"dbServer",
          required: true,
          //2020.08.22配置多個數據庫的DBContext,數據源data的key必須與后臺項目VOL.Core-》EFDbContext下的文件名相同
          data:[]// dbOptions
        }
      ]
    ],
      ...
      [
        {
          ...
          field: 'editType',
          type: 'select',
          data:[{key:0,value:'彈出框編輯'},{key:1,value:'新頁面編輯'}]
        ...
        },
        {
          ...
          field: 'vuePath',
          type: 'text',
          placeholder: 'Vue項目路徑如：E:/app/src/views',
        //  colSize: 6
        },
        {
          title: 'app路徑',
  ...
  columns: [
    ...
    // { field: 'script', title: '腳本', width: 120, align: 'left', editor: 'textarea' },
