...
,master.wf_master_id,master.apply_type,master.Creator,eplmain.glno,eplmain.project_name,ptask.project_task_id,ptask.task_id,ptask.start_date,ptask.end_date,(case when tsk.form_type='0' then 'FormSubmit' else  tsk.form_url end) as form_url,tsk.form_type
	...
	left join cmc_common_task  tsk on ptask.task_id=tsk.task_id   where 1=1     ";
        ...
        public PageGridData<view_pdms_group_epl_fs> GetGroupEplFs(PageDataOptions pageData)
        {
            ...
            QuerySql = @$"        	
	            select  ROW_NUMBER()over(order by {pageData.Sort} {pageData.Order}) as rowId,case when (fs_approve_status='04' or fs_approve_status='' or fs_approve_status is null) then '' else (select  top 1 UserTrueName  from  sys_user where user_id=dev_taker_id) end as submitter,*    
        ...
        public PageGridData<view_pdms_group_epl_fs> GetOrgEplFs(PageDataOptions pageData)
        {
            ...
            QuerySql = @$"        	
	            select  ROW_NUMBER()over(order by {pageData.Sort} {pageData.Order}) as rowId,case when (fs_approve_status='04' or fs_approve_status='' or fs_approve_status is null) then '' else dev_taker_id end as submitter,*    
                       from  cmc_pdms_project_epl 
        ...
        public WebResponseContent ApproveMainPlan(SaveModel saveModel)
            {
                try
                {
                    ...
                    if (task_define != null)
                    {
                        ...
                        saveModel.MainData["epl_id"] = epl_id;
                    }
                    ...
                    if (epl != null)
                    {
                        if (approve_status == "02")
                        {
                            ...
                            WebResponse = Insert_task_hist(saveModel);
                        }
        ...
        {
            var TempList = saveModel.MainData;
            var epl_id = TempList["epl_id"] == null ? Guid.Empty : Guid.Parse(TempList["epl_id"].ToString());
            try
            {
                //查出此epl目前的任務
                var ptask = repository.DbContext.Set<cmc_pdms_project_task>().Where(x => x.epl_id == epl_id).ToList();
            ...
            string deleteAction = @$"
                            ...
                            WHERE
                             epl_id ='{epl_id}' AND action_type = 'delete'";
            try
            {
