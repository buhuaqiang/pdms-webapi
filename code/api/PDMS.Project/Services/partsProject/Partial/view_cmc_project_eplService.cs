...
namespace PDMS.Project.Services
{
    public partial class view_cmc_project_eplService
    {
        ...
        public override PageGridData<view_cmc_project_epl> GetPageData(PageDataOptions options) {
            ...
            {
                QuerySql = @" select  distinct  epl.*,(select UserTrueName   from sys_user where user_id=mset.User_Id) as UserName ,'' as UserTrueName,'' as user_code    
            ...
            { //部門定版
                ...
                QuerySql = @" select  distinct epl.*,(select UserTrueName   from sys_user where user_id=mset.User_Id) as UserName  ,(select  user_code from sys_user where  User_Id=epl.dev_taker_id) as user_code ,(select UserTrueName   from sys_user where user_id=epl.dev_taker_id) as UserTrueName  
	                            from cmc_pdms_project_epl epl
                                left  join cmc_pdms_project_main main on main.project_id=epl.project_id
