...
namespace PDMS.Core.Tenancy
{
    public static class TenancyManager<T> where T : class
    {
        ...
        /// <returns></returns>
        public static string GetSearchQueryable(string multiTenancyString, string tableName)
        {
            if (DBType.Name == DbCurrentType.PgSql.ToString())
            {
                tableName = $" public.\"{tableName}\" ";
            }
            if (multiTenancyString == null)
            {
                multiTenancyString = $"select * from {tableName}";
            }
            else
            {
                multiTenancyString = $"select * from （{tableName}） as t";
            }
